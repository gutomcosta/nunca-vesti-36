...
$sDateAndTimeFormat = get_option( 'date_format' ).' '.get_option( 'time_format' ); ?>
    ...
    ?>
            ...
            <?php if ( $i == 1 ) { ?>
			<div class="masonryBox clear">
            <?php } ?>

                <?php if ( ( $i != 1 && !$bCustomContentDefined ) || $bCustomContentDefined ) { ?>
				<div id="post-<?php the_ID(); ?>" <?php post_class('blogPostItem') ?>>
					<a href="<?php the_permalink() ?>" class="blogPostImg">
                        <?php if ( has_post_thumbnail() ) { ?>
                            <?php the_post_thumbnail( 'unithumb-blogmasonry', array( 'alt' => the_title_attribute('echo=0') ) ); ?>
                        <?php } else { ?>
						    <img src="http://placehold.it/370x247/8acace/ffffff" alt="<?php the_title_attribute() ?>" width="370" height="247">
                        <?php } ?>
					</a>
